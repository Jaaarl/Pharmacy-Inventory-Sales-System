...
- Current state: the pure calculation service and calculation-level acceptance tests are implemented and passing. F05 atomic snapshot persistence, serialization/Room round-trip tests, and stored historical-version checks remain open; do not mark F03 fully verified until those are complete.
