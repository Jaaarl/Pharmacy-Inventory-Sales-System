...
The repository contains a Kotlin/Compose Android compatibility scaffold, Room databases, F01 authentication/authorization, F02 catalog/import code, and F06 inventory workflows. F02 has a connected create/edit/inactivate UI, protected and effective-dated field changes, CSV preview and reviewed-subset commit, opening-lot ledger movements, and an additive prescription-class migration. F06 has ledger-derived stock and lot balances, protected receiving and lot-scoped adjustments, exact SKU-specific pack conversion, signed outgoing movements, expired-lot exclusion and disposal, low-stock/expiry warnings with an explicit near-expiry horizon, cost snapshots, audit evidence, and an inventory screen with recent movements. F03 now has a pure Kotlin line-level calculation engine with centavo money, explicit approved rounding and versioned rate inputs, effective-dated catalog snapshots, VAT/SC/PWD treatment, explicit promotion selection/stacking, mixed-cart aggregation, and BNPC disabled until a complete approved capped policy is supplied. F04 checkout captures SC/PWD identity evidence, checks the physical ID, applies benefits only to explicitly selected eligible lines, and encrypts the customer name/full ID with Android Keystore. F05 adds Room schema version 6, atomic sale/line/snapshot/settlement/FEFO movement/audit writes, offline Manila-date transaction IDs, a UUID idempotency key, and finalized-record immutability. On 2026-09-24, `:app:testDebugUnitTest` passed with 60 tests and no failures when Gradle ran on JDK 25 and test workers used the installed Java 21 runtime (required by the current Robolectric version). The F04/F05 verification includes all injected transaction checkpoints, concurrent last-unit attempts, mixed carts, SC/PWD, cash/QR declarations, sequential IDs, migration SQL, snapshot round-trip, and retry idempotency. Compose UI automation and physical MatePad keystore/restart/reboot checks remain gates; the owner-reported Phase 0 compatibility result is not a substitute for feature-level evidence. F06 component stock remains with F07, return/reversal integration with F09, and inventory reports with F10.
...
- Current state: the pure calculation service, F05 atomic snapshot persistence, versioned serialization, and codec round-trip test are implemented and passing. Physical-device and end-to-end stored-history checks remain part of the Phase 1 verification gate.
...
**Current state (2026-09-24):** The checkout UI and application service are implemented. The full JVM suite passes (60 tests); F04/F05 tests cover SC/PWD, mixed carts, identity evidence, CASH/QR, idempotency, unique sequences, FEFO deductions, last-unit concurrency, all transaction failure checkpoints, migration creation, and snapshot serialization. Compose UI automation and physical MatePad restart/reboot/keystore checks remain open. This package is implemented, but its phase gate is not verified until those remaining checks are recorded.
